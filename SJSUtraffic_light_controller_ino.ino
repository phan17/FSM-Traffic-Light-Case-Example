...
// Define States
enum State {NS1_GREEN, NS1_YELLOW, NS2_GREEN, NS2_YELLOW, 
ALL_RED, EW1_GREEN, EW1_YELLOW, EW2_GREEN, EW2_YELLOW, PEDESTRIAN};
State currentState = NS1_GREEN;
State nextStateAfterAllRed = EW1_GREEN;
...
const int NS2_RED = 35, NS2_YEL = 37, NS2_GRN = 39;
const int EW1_RED = 28, EW1_YEL = 30, EW1_GRN = 32;
const int EW2_RED = 40, EW2_YEL = 42, EW2_GRN = 44;
const int PED_LED = 50;

const int EMER_PIN = 2;
const int PED_PIN = 49;
  ...
  switch (currentState) {
    ...
    case NS1_YELLOW:
      ...
      if (now - stateMillis >= YELLOW_TIME) {
        nextStateAfterAllRed = NS2_GREEN;
    ...
    case NS2_YELLOW:
      ...
      setSignal(EW2_RED, EW2_YEL, EW2_GRN, true, false, false);
      if (now - stateMillis >= YELLOW_TIME) {
        nextStateAfterAllRed = EW1_GREEN;
        currentState = ALL_RED;
        stateMillis = now;
      }
      break;
    ...
    case EW1_YELLOW:
      ...
      setSignal(EW2_RED, EW2_YEL, EW2_GRN, true, false, false);
      if (now - stateMillis >= YELLOW_TIME) {
        nextStateAfterAllRed = EW2_GREEN;
        currentState = ALL_RED;
        stateMillis = now;
      }
      break;
    ...
    case EW2_YELLOW:
      ...
      if (now - stateMillis >= YELLOW_TIME) {
        nextStateAfterAllRed = NS1_GREEN;
        currentState = ALL_RED;
        stateMillis = now;
      }
      break;
    ...
    case ALL_RED:
      ...
      if (now - stateMillis >= ALL_RED_TIME) {
        if (pedRequest && nextStateAfterAllRed == NS1_GREEN) {
